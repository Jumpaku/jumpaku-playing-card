...
export function verifyJWT<P extends {}>(
  jwt: string,
  pubKey: string,
  params: {
    timestamp: Date;
...
export function issueJWT<P extends {}>(
  payload: P,
  secKey: string,
  params: {
  ...
  if (params.expiresAt != null) {
    Object.assign(options, {
      expiresIn: Math.floor(
        (params.expiresAt.getTime() - params.issuedAt.getTime()) / 1000,
      ),
    });
  }
  return sign(
    { ...payload, iat: instant(params.issuedAt).epochSeconds },
    secKey,
    options,
  );
}
