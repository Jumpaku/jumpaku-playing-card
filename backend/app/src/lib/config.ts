...
export function overwriteConfigWithJson<Desc extends DescMessage>(
  schema: Desc,
  config: MessageShape<Desc>,
  overwrite: JsonObject,
) {
  return mergeFromJson(schema, config, overwrite);
}

export function overwriteConfigWithEnv<Desc extends DescMessage>(
  schema: Desc,
  config: MessageShape<Desc>,
  ...
  walkField(schema, (fieldPath, field) => {
    ...
    if (field.fieldKind === 'enum') {
      const v = field.enum.values.find(
        (v) => v.name === envValue || v.number.toString() === envValue,
      );
      if (v === undefined) {
    ...
    if (field.fieldKind === 'scalar') {
      if (field.scalar === ScalarType.BOOL) {
        ...
        setObjectValue(
          overwrite,
          fieldPath,
          ['true', '1', 't'].includes(envValue.toLowerCase()),
        );
      } else {
...
function walkField(
  schema: DescMessage,
  f: (fieldPath: string[], s: DescField) => void,
  fieldPath: string[] = [],
) {
