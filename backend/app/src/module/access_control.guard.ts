...
import {
  JwtPayload_JwtKind,
  JwtPayload_JwtKindSchema,
  JwtPayloadJson,
} from '../gen/pb/api/v1/jwt_pb';
import { TokenExpiredError } from 'jsonwebtoken';
import { enumToJson } from '@bufbuild/protobuf';
import {
  throwAccessTokenExpired,
  throwRefreshTokenExpired,
  ...
  canActivate(
    ...
    try {
      ...
      if (e instanceof TokenExpiredError) {
        ...
        if (
          payload?.kind ===
          enumToJson(JwtPayload_JwtKindSchema, JwtPayload_JwtKind.ACCESS)
        ) {
          throwAccessTokenExpired(
        ...
        if (
          payload?.kind ===
          enumToJson(JwtPayload_JwtKindSchema, JwtPayload_JwtKind.REFRESH)
        ) {
          throwRefreshTokenExpired(
    ...
    const scopes =
      payload.kind === 'JWT_KIND_ACCESS'
        ? (payload.accessData?.scopes ?? [])
        : payload.kind === 'JWT_KIND_REFRESH'
          ? (payload.refreshData?.scopes ?? [])
          : [];
